...
class AddToCartView(LoginRequiredMixin, View):
    def post(self, request, book_id):
        ...
        # Calculate updated values for cart summary
        return JsonResponse({
            'success': True,
...
class CheckoutView(LoginRequiredMixin, TemplateView):
    ...
    def post(self, request):
        ...
        # Add books to user's owned collection (LibraryBook model only - not borrowed)
        for cart_item in cart.items.all():
            ...
            if library_book:
                ...
                # For purchased books, set a far future due date (30 years)
                far_future_date = timezone.now() + datetime.timedelta(days=365*30)  # 30 years in the future
                LibraryBook.objects.create(
                    user=request.user,
                    book=cart_item.book,
                    status='owned',  # Mark as owned not borrowed
                    due_date=far_future_date,  # Required due_date field but we use a far future date
        ...
        messages.success(request, f'Purchase complete! {total_books} books added to your owned collection.')
...
class BorrowView(LoginRequiredMixin, View):
    def post(self, request):
        ...
        for cart_item in cart.items.all():
            if cart_item.book.stock < cart_item.quantity:
                messages.error(request, f'Not enough copies of "{cart_item.book.title}" available for borrowing.')
                return redirect('cart:cart')
        ...
        # Add books to user's borrowed collection (BorrowedBook model)
        for cart_item in cart.items.all():
            ...
            if existing_borrowed:
                ...
                existing_borrowed.quantity += cart_item.quantity
        ...
        messages.success(request, f'Successfully borrowed {total_books} books. They will be due in 14 days.')
        return redirect('books:library')
