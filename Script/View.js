document.addEventListener('DOMContentLoaded', function () {
    ...
    viewedBook.id = viewedBook.id || viewedBook.title.replace(/\s+/g, '-').toLowerCase();
  
    document.getElementById('BookCover').src = viewedBook.image || 'Style/Images/Plain Book.png';
    ...
    bookInfoDiv.innerHTML = `
      ...
      <p><strong>Description:</strong> ${viewedBook.description || 'No description available'}</p>
    `;
  
    const currentUser = JSON.parse(localStorage.getItem('CurrentUser')) || { books: [] };
    const alreadyOwned = currentUser.books.some(book => book.title === viewedBook.title);
    if (alreadyOwned) {
    ...
    alreadyOkButton.addEventListener('click', () => alreadyModal.style.display = 'none');
  });
