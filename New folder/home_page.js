...
document.addEventListener('DOMContentLoaded', function () {
  ...
  displayBooksByCategory(books, 'popular-books-grid', ['Fiction', 'Non-Fiction', 'Science Fiction']);
  displayBooksByCategory(books, 'trending-books-grid', ['Fantasy', 'Mystery', 'Thriller']);
  displayBooksByCategory(books, 'new-books-grid', ['Romance', 'Horror', 'Action']);
  ...
  function addViewDetailsListeners() {
    document.querySelectorAll('.view-book-details').forEach(link => {
      link.addEventListener('click', function (e) {
        ...
        const author = (paragraphs.find(p => p.textContent.startsWith('Author:'))?.textContent || '').replace('Author: ', '');
        const stock = (paragraphs.find(p => p.textContent.startsWith('Stock:'))?.textContent || '').replace('Stock: ', '');
        const price = (paragraphs.find(p => p.textContent.startsWith('Price:'))?.textContent || '').replace('Price: ', '');
  ...
  function displayBooksByCategory(books, gridId, categories) {
    ...
    const filteredBooks = books.filter(book => categories.includes(book.genre));
    const booksToDisplay = filteredBooks.slice(0, 4);
  
    if (booksToDisplay.length === 0) {
      grid.innerHTML = '<div class="no-books">No books available in this category</div>';
      return;
    }
    ...
    booksToDisplay.forEach(book => {
      ...
      bookCard.innerHTML = `
        <img src="${book.image || 'Style/Images/default-book.jpg'}" alt="${book.title}">
        <h3>${book.title}</h3>
  ...
  function initSliders() {
    ...
    sliders.forEach(slider => {
      ...
      function updateSlider() {
        ...
        rightArrow.style.display = currentSlide === slideCount - 1 ? 'none' : 'block';
